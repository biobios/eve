...
- `src/chat.ts`: 458行 - チャットロジック、セッション管理UI

#### バックエンド (TypeScript)
- `src/database-migrations-config.ts`: 261行 - データベースマイグレーション定義
- `src/database-migration.ts`: 284行 - マイグレーション管理システム
- `src/crypto-utils.ts`: 257行 - 暗号化とAPIキー保存
- `src/database-manager.ts`: 214行 - データベース統合管理
- `src/ipc-handlers.ts`: 213行 - IPC通信ハンドラー集約
- `src/session-storage.ts`: 180行 - セッションデータ永続化
- `src/ai-manager.ts`: 103行 - AI統合、LangGraphワークフロー
- `src/main.ts`: 98行 - アプリ起動、コンポーネント統合
- `src/conversation-manager.ts`: 97行 - 会話管理ロジック
- `src/window-manager.ts`: 92行 - ウィンドウ管理、セキュリティ設定
- `src/preload.ts`: 79行 - セキュアAPI公開
- `src/ai-service.ts`: 70行 - AI処理サービス
- `src/session-manager.ts`: 70行 - セッション状態管理

#### ルートファイル
- `main.js`: 40行 - Electronエントリーポイント

**合計プロジェクトコード行数**: 約2,960行
...
2. **apikeys.db**: APIキー管理  
   - テーブル: `api_keys`
   - フィールド: `service_name`, `encrypted_api_key`, `description`, `is_active`, `last_used_at`
   - インデックス: `service_name`
   - マイグレーション: 3バージョン
